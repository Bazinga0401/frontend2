...
addTaskForm.onsubmit = async e => {
  ...
  const taskObj = {
    ...
    time: taskTime.value,
    week: currentTab,
    subbatch: userSubbatch  // ✅ Comes from logged-in user
  };
  ...
  try {
    ...
    if (!data.success) {
      return alert('Failed to save task in DB.');
    }

    const newTask = {
      ...taskObj,
      day: dayIdx,
      id: data.task._id  // ✅ MongoDB ObjectId
    };
...
async function fetchTasksFromDB() {
  try {
    ...
    const data = await res.json();
    console.log('Fetched task data:', data); // for debugging
    ...
    if (data.success) {
      ...
      data.tasks.forEach(task => {
        ...
        target[task.day].push({ ...task, id: task._id });
      });
